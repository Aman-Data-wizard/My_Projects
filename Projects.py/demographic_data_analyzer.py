...
def calculate_demographic_data(print_data=True):
    # Read data from file
    df = pd.read_csv('D:\\Python\\Projects.py\\adult.data.csv')  # Ensure your file path is correct[8][10]
    ...
    # Percentage of these minimum-hour workers who earn >50K
    min_workers = df['hours-per-week'] == min_work_hours
    rich_percentage = round((df[min_workers]['salary'] == '>50K').sum() / min_workers.sum() * 100, 1)

    # What country has the highest percentage of people that earn >50K?
    country_percentages = (df[df['salary'] == '>50K']['native-country'].value_counts() /
